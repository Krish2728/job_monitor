import { containsAny, containsAnyBoundary, normalizeText } from './normalize.js';
import { experienceWithinLimit } from './experience.js';
...
function cityMatches(locationBlob, city) {
  const normalizedCity = city.toLowerCase();
  const aliases = CITY_ALIASES[normalizedCity] || [normalizedCity];
  return aliases.some((alias) => locationBlob.includes(alias));
}
...
function locationMatches(job, company, globalFilters) {
  ...
  if (companyCities.length > 0) {
    if (companyCities.some((city) => cityMatches(locationBlob, city))) {
      return true;
    }
...
function titleExcluded(title, company, globalFilters) {
  ...
  return containsAnyBoundary(title, excludes);
}
...
export function scoreJob(job, company, globalFilters) {
  ...
  const maxExperienceYears = globalFilters.max_experience_years ?? 2;
  const experienceCheck = experienceWithinLimit(job, maxExperienceYears);
  if (!experienceCheck.ok) {
    return {
      matched: false,
      score,
      reasons: [`experience requirement ${experienceCheck.minRequired}+ years`],
    };
  }
  if (experienceCheck.minRequired !== null) {
    score += 1;
    reasons.push(`experience <= ${maxExperienceYears} years`);
  }
